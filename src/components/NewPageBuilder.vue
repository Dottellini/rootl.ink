<template>
  <div :style="PageBuilderStyle" id="PageBuilder">
    <canvas id="imageCanvas" hidden></canvas> <!-- Der wird für das hochladen der bilder benötigt. Wichtig! -->
            ...
            <draggable :list="list" :disabled="!enabled" :animation="200" handle=".handle" class="list-group" ghost-class="ghost" drag-class="drag" chosen-class="chosen" fallbackClass="sortable-fallback" @start="dragging = true">
              <div class="list-group-item" v-for="element in list" :key="element.id">
                <div class="horizontal-container">
                  ...
                  <div>
                    <input type="file" :id="`${element.id}_u`" accept="image/*" @change="changeImage(element.id, 'image/jpeg', $event, 'link')" hidden>
                    <label :for="`${element.id}_u`" class="upload-image">
                      <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-upload" viewBox="0 0 16 16">
                        <path d="M.5 9.9a.5.5 0 0 1 .5.5v2.5a1 1 0 0 0 1 1h12a1 1 0 0 0 1-1v-2.5a.5.5 0 0 1 1 0v2.5a2 2 0 0 1-2 2H2a2 2 0 0 1-2-2v-2.5a.5.5 0 0 1 .5-.5z"/>
                        <path d="M7.646 1.146a.5.5 0 0 1 .708 0l3 3a.5.5 0 0 1-.708.708L8.5 2.707V11.5a.5.5 0 0 1-1 0V2.707L5.354 4.854a.5.5 0 1 1-.708-.708l3-3z"/>
                      </svg>
                    </label>
                    <button @click="deleteImage(element.id)" class="delete-button" style="margin-left: 5px;">
                  ...
                  <div>
                    ...
                    <label :for="`${element.id}_h`" class="embed">Highlight</label>
                    <input :id="`${element.id}_h`" type="checkbox" v-model="element.highlight">
                    <div v-if="element.isYoutubeVideo">
                      <label :for="`${element.id}_y`" class="embed">Embed Video</label>
                      <input :id="`${element.id}_y`" type="checkbox" v-model="element.embed">
                    </div>
...
export default {
  ...
  methods: {
    deleteImage(id) {
      this.$store.commit("removeImageFromEntry", id)
    },
